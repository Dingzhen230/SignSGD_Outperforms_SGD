...
def get_wikitext_data(datasets_dir, num_proc=10):
    """To change the cache dir, run `export HF_HOME=/path/to/cache/` before running the code."""

    WIKITEXT_DATA_PATH = os.path.join(datasets_dir, "wikitext/")
    if not os.path.exists(os.path.join(WIKITEXT_DATA_PATH, "train.bin")):
        ...
        split_dataset = dataset.train_test_split(
            test_size=0.001, seed=2357, shuffle=True
        )

...
def eval_and_log(
    ...
    if distributed_backend.is_master_process():
        ...
        val_loss_file = csv_dir / "val_loss.csv"
        val_acc_file = csv_dir / "val_acc.csv"
        
        # 追加写入验证损失
        with open(val_loss_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([curr_iter, f"{val_loss:.6f}"])
        
        # 追加写入验证准确率
        with open(val_acc_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([curr_iter, f"{val_acc:.6f}"])
    model.train()
...
def train(
    ...
    if distributed_backend.is_master_process():
        ...
        # 初始化 CSV 文件
        train_loss_file = csv_dir / "train_loss.csv"
        val_loss_file = csv_dir / "val_loss.csv" 
        val_acc_file = csv_dir / "val_acc.csv"
        ...
        with open(val_loss_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['iteration', 'val_loss'])
        with open(val_acc_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['iteration', 'val_acc'])
    ...
    while curr_iter <= cfg.iterations:
        ...
        # if curr_iter in noise_analysis_steps:
        #     ckpt_dir = exp_dir / "ckpts" / f"itr_{curr_iter}"
        #     if distributed_backend.is_master_process():
        #         save_checkpoint(model, opt, scheduler, curr_iter, ckpt_dir)
        #     save_worker_state(ckpt_dir)
        ...
        if (
            ...
            train_loss = loss.detach().item() * cfg.acc_steps
            train_aux_losses = {
            ...
            if distributed_backend.is_master_process():
                ...
                with open(train_loss_file, 'a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([curr_iter, f"{train_loss:.6f}"])
